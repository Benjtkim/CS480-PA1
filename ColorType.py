...
class ColorType:
    ...
    def __add__(self, other: ColorType) -> ColorType:
        return ColorType(self.r + other.r, self.g + other.g, self.b + other.b)

    def __sub__(self, other: ColorType) -> ColorType:
        return ColorType(self.r - other.r, self.g - other.g, self.b - other.b)

    def __mul__(self, scalar: float) -> ColorType:
        return ColorType(self.r * scalar, self.g * scalar, self.b * scalar)

    def __rmul__(self, scalar: float) -> ColorType:
        return self.__mul__(scalar)

    def setRGB(self, r: float = 0, g: float = 0, b: float = 0) -> None:
        """
        This method will check v values to make sure they are in range.
        Chained comparisons are faster than nested min/max
        ...
        :rtype: None
        """
        self.r = r if 0.0 <= r <= 1.0 else (0.0 if r < 0.0 else 1.0)
        self.g = g if 0.0 <= g <= 1.0 else (0.0 if g < 0.0 else 1.0)
        self.b = b if 0.0 <= b <= 1.0 else (0.0 if b < 0.0 else 1.0)

    def setRGB_8bit(self, r: int = 0, g: int = 0, b: int = 0) -> None:
        """
    ...
    def setRGB_ARGB(self, argb: int) -> None:
        """
    ...
    def setRGB_RGBA(self, rgba: int) -> None:
        """
    ...
    def getRGB_8bit(self) -> tuple[int, int, int]:
        """
